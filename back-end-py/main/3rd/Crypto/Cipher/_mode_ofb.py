...
class OfbMode(object):
    ...
    def __init__(self, block_cipher, iv):
        ...
        if result := raw_ofb_lib.OFB_start_operation(
            block_cipher.get(),
            c_uint8_ptr(iv),
            c_size_t(len(iv)),
    ...
    def encrypt(self, plaintext, output=None):
        ...
        if result := raw_ofb_lib.OFB_encrypt(
            self._state.get(),
    ...
    def decrypt(self, ciphertext, output=None):
        ...
        if result := raw_ofb_lib.OFB_decrypt(
            self._state.get(),
            c_uint8_ptr(ciphertext),
            c_uint8_ptr(plaintext),
...
def _create_ofb_cipher(factory, **kwargs):
    ...
    if kwargs:
        raise TypeError(f"Unknown parameters for OFB: {kwargs}")
