...
class CfbMode(object):
    ...
    def __init__(self, block_cipher, iv, segment_size):
        ...
        if result := raw_cfb_lib.CFB_start_operation(
            block_cipher.get(),
            c_uint8_ptr(iv),
            c_size_t(len(iv)),
    ...
    def encrypt(self, plaintext, output=None):
        ...
        if result := raw_cfb_lib.CFB_encrypt(
            self._state.get(),
    ...
    def decrypt(self, ciphertext,  output=None):
        ...
        if result := raw_cfb_lib.CFB_decrypt(
            self._state.get(),
            c_uint8_ptr(ciphertext),
            c_uint8_ptr(plaintext),
...
def _create_cfb_cipher(factory, **kwargs):
    ...
    if kwargs:
        raise TypeError(f"Unknown parameters for CFB: {kwargs}")
